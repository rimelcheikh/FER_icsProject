...
class CohnKanade(Dataset):
    ...
    def _load(self):
        # Setting the training, validation and test indexes
        ...
        for subject in subjects:
            ...
            for session in sessions:
                ...
                if len(file_class) > 0:
                    ...
                    image_is = io.imread(path_frames + frames[-4])
                    if (np.shape(image_is) == (490, 640)):
                        folds_data_index[subject_step].append(index_image)
                        index_image += 1
                        data.append(image_is)
        ...
        folds_data_index = np.array(folds_data_index, dtype=object)
...
class AffectNetCategorical(Dataset):
    ...
    def _load(self):
        ...
        # Randomize folders
        if self.idx_set < 2:
            np.random.shuffle(images_folder_affect_net)
            np.random.shuffle(labels_folder_affect_net)
